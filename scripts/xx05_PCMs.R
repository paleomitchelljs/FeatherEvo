...
Traits <- ModelDat[Rows,c("mass", "wingR", "tailR", "tarsusR", "rachisWL", "barbDR", "barbLR", "barbAng", "barbL_lead")]
...
for (i in 1:length(strees))	{
	for (j in 1:ncol(Traits))	{
		useTree <- dwtrees[[i]]
		useDat <- data.frame(rownames(Traits), RawDat$flightless, Traits[,j])

		BMs <- OUwie(useTree, useDat, model="BMS", simmap.tree=TRUE, root.station = FALSE)
		lessMat[i,j] <- BMs$solution[2,1]
		volMat[i,j] <- BMs$solution[2,2]
		aiccMat[i,j] <- BMs$AICc

		BM <- OUwie(useTree, useDat, model="BM1", simmap.tree=TRUE, root.station = FALSE)
