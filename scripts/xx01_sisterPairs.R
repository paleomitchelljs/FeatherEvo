...
# Do all traits
sCol <- which(colnames(RawDat) == "mass")
eCol <- ncol(RawDat)
nCols <- length(sCol:eCol)

# Choose traits
RawDat$barbuleDR <- RawDat$barbule_density_proximal / RawDat$barbule_density_distal
Alt <- c("mass", "wingR", "tailR", "tarsusR", "rachisWL", "barbDR", "barbLR", "barbL_lead", "barbL_trail", "barbuleL_distrail", "barbAng", "rachis_width", "barb_density_l", "barb_density_t", "barb_length_l", "barb_length_t", "barb_angle_l", "barb_angle_t", "barbuleDR")
nCols <- length(Alt)
...
#	sisMat_vol[i,] <- c(i,unlist(RawDat[which(RawDat$species == NamesFlight[which(NamesFlight[,2]==1),1]),sCol:eCol]))
	sisMat_vol[i,] <- c(i,unlist(RawDat[which(RawDat$species == NamesFlight[which(NamesFlight[,2]==1),1]),Alt]))
...
#	sisMat_less[i,] <- c(i,unlist(RawDat[which(RawDat$species == NamesFlight[which(NamesFlight[,2]==0),1]),sCol:eCol]))
	sisMat_less[i,] <- c(i,unlist(RawDat[which(RawDat$species == NamesFlight[which(NamesFlight[,2]==0),1]),Alt]))
...
plot(sisterTree, cex=0.45)
for (trait in 2:ncol(sisMat_less))	{
	Ylim <- range(c(sisMat_vol[,trait],sisMat_less[,trait]), na.rm=T, finite=T)

#	par(las=1, mgp=c(1.5, 0.5, 0), tck=-0.01)
	plot(1, 1, type="n", xlim=c(0, 1), xaxt="n", ylim=Ylim, xlab="", ylab="", main=colnames(sisMat_less)[trait])
