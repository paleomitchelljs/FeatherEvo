...
makeReg <- function(x, y, flight, regression, Ndraw = 1e2, Xlab="", Ylab="", Xline = 1.5, Yline = 2, labcex = 1)	{
	Opac <- 10 / Ndraw
	Cols <- c(rgb(241/255,163/255,64/255,Opac),rgb(153/255,142/255,195/255,Opac))
	plot(x, y, pch=c(22,21)[flight+1], bg=Cols[flight+1], xlim=range(pretty(x)), ylim=range(pretty(y)), cex=1.5, xlab="", ylab="", type="n")
	...
	legend("topleft", bty="n", legend=c("Volant", "Flightless"), pch=c(15,16), col=c(rgb(241/255,163/255,64/255,1),rgb(153/255,142/255,195/255,1)))
}
...
Labcex <- 0.75
Labs <- c("Volant", "Flightless")
OMACex <- 1.15
OMALine <- 1
boxplot(RawDat$barbDR~RawDat$flightless, boxwex=0.25, border=Cols, col="white", lty=1, ylab="", xlab="", xaxt="n", yaxt="n", lwd=LWD)
...
makeReg(RawDat$barb_density_t, RawDat$barb_density_l, flight = RawDat$flightless, regression = barb_dens, Xlab="Trail barb density (#/mm)", Ylab = "Lead barb density (#/mm)", labcex = Labcex)
mtext("Phylogenetic\nRegressions", side = 3, line = OMALine, font=2, outer = F, cex = OMACex)

makeHist(barb_dens, Xlab = expression(paste("log Estimated rate (", gamma, ")", sep="")), Ylab = "Proportion", labcex = Labcex)
mtext("Evolutionary Rate\nComparisons", side = 3, line = OMALine, font=2, outer = F, cex = OMACex)


### Barb LR
boxplot(RawDat$barbLR~RawDat$flightless, boxwex=0.25, border=Cols, col="white", lty=1, ylab="", xlab="", xaxt="n", yaxt="n", lwd=LWD)
...
makeHist(barb_len, Xlab = expression(paste("log Estimated rate (", gamma, ")", sep="")), Ylab = "Proportion", labcex = Labcex)

### Barb Angle
boxplot(RawDat$barbAng~RawDat$flightless, boxwex=0.25, border=Cols, col="white", lty=1, ylab="", xlab="", xaxt="n", yaxt="n", lwd=LWD)
...
makeHist(barb_ang, Xlab = expression(paste("log Estimated rate (", gamma, ")", sep="")), Ylab = "Proportion", labcex = Labcex)


### Wing
boxplot(RawDat$wingR~RawDat$flightless, boxwex=0.25, border=Cols, col="white", lty=1, ylab="", xlab="", xaxt="n", yaxt="n", lwd=LWD)
...
makeHist(wing, Xlab = expression(paste("log Estimated rate (", gamma, ")", sep="")), Ylab = "Proportion", labcex = Labcex)
